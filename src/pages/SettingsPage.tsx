...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border border-zinc-700/25 rounded-lg bg-zinc-800/20 overflow-hidden">
      <div className="px-5 py-3 border-b border-zinc-700/25 bg-zinc-800/30">
        <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-400 font-semibold">{title}</p>
      </div>
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 transition-colors ${
        checked ? 'border-amber-500/60 bg-amber-500/25' : 'border-zinc-600 bg-zinc-700/40'
      }`}
    >
      <span
        className={`pointer-events-none inline-block h-3.5 w-3.5 rounded-full bg-zinc-200 shadow transition-transform mt-[1px] ${
          checked ? 'tranzinc-x-[14px]' : 'tranzinc-x-[1px]'
        }`}
      />
    </button>
  )
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-zinc-800/40 border border-zinc-700/40 rounded-md px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 outline-none focus:border-amber-500/40 transition-colors"
                />
                <button
          ...
          <Section title="Account Security">
            {/* Change email */}
            <div>
              <label className="text-xs text-zinc-400 mb-1.5 block">Change Email</label>
              <div className="flex gap-2">
                <input
                  type="email"
                  value={newEmail}
            ...
            {/* Change password */}
            <div>
              <label className="text-xs text-zinc-400 mb-1.5 block">Change Password</label>
              <div className="space-y-2">
                <input
                  ...
                  className="w-full bg-zinc-800/40 border border-zinc-700/40 rounded-md px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 outline-none focus:border-amber-500/40 transition-colors"
                />
                <div className="flex gap-2">
                  <input
                    type="password"
          ...
          <Section title="Appearance">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-0.5">Apply the dark theme across the entire app</p>
              </div>
          ...
          <Section title="Danger Zone">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="px-4 py-2 rounded-md bg-zinc-700/40 border border-zinc-600/40 text-zinc-300 text-sm hover:bg-zinc-700/60 transition-colors"
              >
                Sign Out
              </button>
            </div>
            ...
            {/* Delete account */}
            <div className="border-t border-zinc-700/25 pt-4">
                ...
                <div className="rounded-md border border-red-500/25 bg-red-500/5 p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-zinc-800/60 border border-red-500/25 rounded-md px-3 py-2 text-sm text-red-300 placeholder-zinc-600 outline-none font-mono"
                    />
                    <button
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-md border border-zinc-700/40 text-zinc-400 text-sm hover:bg-zinc-700/30 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
