...
function triggerColor(kind: Trigger['kind']): string {
  return TRIGGER_KINDS.find(k => k.value === kind)?.color ?? 'bg-slate-500/20 text-slate-400 border-slate-500/30'
}
...
export default function CharacterNotePanel({ campaignId, characterId }: CharacterNotePanelProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Profile note */}
      <div>
        <h4 className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Profile Note</h4>
        <textarea
          value={profileText}
          onChange={e => handleProfileChange(e.target.value)}
          placeholder="Freeform DM notes about this character..."
          rows={4}
          className="w-full bg-slate-900/50 border border-slate-700/30 rounded px-3 py-2 text-xs text-slate-200 placeholder-slate-600 resize-none focus:outline-none focus:border-amber-500/30"
        />
      </div>

      {/* Triggers */}
      <div>
        <h4 className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Triggers</h4>
            ...
            {sortedTriggers.map(t => (
              <div key={t.id} className="flex items-center gap-1.5">
                <button
                  ...
                  className={`text-[9px] ${t.pinned ? 'text-amber-400' : 'text-slate-600'} hover:text-amber-300 transition-colors`}
        ...
        <div className="flex gap-1.5">
          <input
            ...
            className="flex-1 bg-slate-900/50 border border-slate-700/30 rounded px-2 py-1 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500/30"
            onKeyDown={e => e.key === 'Enter' && handleAddTrigger()}
          />
          <select
            value={newTriggerKind}
            onChange={e => setNewTriggerKind(e.target.value as Trigger['kind'])}
            className="bg-slate-900/50 border border-slate-700/30 rounded px-1.5 py-1 text-[10px] text-slate-400 focus:outline-none"
          >
            {TRIGGER_KINDS.map(k => (
      ...
      {charMoments.length > 0 && (
        <div>
          <h4 className="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Moments</h4>
...
function MomentEntry({ moment }: { moment: SessionMoment }) {
  ...
  return (
    <div className="flex items-start gap-2 text-xs">
      <span className={`text-[9px] shrink-0 mt-0.5 ${kindColors[moment.kind] ?? 'text-slate-400'}`}>
        {moment.kind.replace('_', ' ')}
      </span>
      <span className="text-slate-300">{moment.text}</span>
      <span className="text-[9px] text-slate-600 shrink-0 ml-auto">
