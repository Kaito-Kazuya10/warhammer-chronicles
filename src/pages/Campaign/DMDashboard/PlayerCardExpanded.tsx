...
function StatRow({ label, value, color }: { label: string; value: string | number; color?: string }) {
  return (
    <div className="flex items-center justify-between py-1 border-b border-zinc-800/30">
      ...
      <span className={`text-xs font-mono ${color ?? 'text-zinc-300'}`}>{value}</span>
    </div>
  )
}
...
export default function PlayerCardExpanded({ character, campaignId, onClose }: PlayerCardExpandedProps) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-80 bg-[#1a1d22] border-l border-zinc-700/30 shadow-2xl z-50 flex flex-col">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-700/20">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {character.portrait ? (
            <img src={character.portrait} alt="" className="w-10 h-10 rounded-full object-cover border border-zinc-700/30" />
          ) : (
            <div className="w-10 h-10 rounded-full bg-zinc-800/50 border border-zinc-700/30 flex items-center justify-center text-xs font-bold text-zinc-600">
              {character.name.slice(0, 2).toUpperCase()}
            </div>
          )}
          <div className="min-w-0">
            <p className="text-sm font-medium text-zinc-200 truncate">{character.name}</p>
      ...
      {/* Tabs */}
      <div className="flex border-b border-zinc-700/20">
        <button
          onClick={() => setActiveTab('stats')}
          className={`flex-1 py-2 text-[10px] tracking-widest uppercase transition-colors ${
            activeTab === 'stats' ? 'text-amber-400 border-b border-amber-400' : 'text-zinc-600 hover:text-zinc-400'
          }`}
        >
          Stats
        </button>
        <button
          ...
          className={`flex-1 py-2 text-[10px] tracking-widest uppercase transition-colors ${
            activeTab === 'notes' ? 'text-amber-400 border-b border-amber-400' : 'text-zinc-600 hover:text-zinc-400'
          }`}
        >
          Notes
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        ...
        {/* HP */}
        <div>
          <h4 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Hit Points</h4>
          <div className="h-2 rounded-full bg-zinc-900/50 overflow-hidden mb-1">
            <div
          ...
          <div className="flex gap-3 text-xs font-mono">
            <span className="text-zinc-300">{character.currentHitPoints}/{character.maxHitPoints}</span>
          ...
          <div className="mt-2 flex gap-1.5">
            <input
              ...
              placeholder="Amount"
              className="flex-1 bg-zinc-900/50 border border-zinc-700/30 rounded px-2 py-1 text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-amber-500/30 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
              onKeyDown={e => {
        ...
        {/* Class resources */}
        <div>
          <h4 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Class Resources</h4>
        ...
        {/* Combat */}
        <div>
          <h4 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Combat</h4>
        ...
        {/* Death saves */}
        <div>
          <h4 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">Death Saves</h4>
