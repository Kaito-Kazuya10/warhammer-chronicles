      ...
      className={`flex items-center gap-3 px-3 py-2 rounded-md border transition-colors ${
        isNat20
          ...
          : isNat1
            ...
            : 'bg-zinc-800/30 border-zinc-700/15'
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-[10px] text-zinc-600">{roll.diceExpression}</span>
      </div>
      <span
        className={`text-sm font-mono font-bold shrink-0 ${
          isNat20 ? 'text-amber-400' : isNat1 ? 'text-red-400' : 'text-zinc-200'
        }`}
      >
        {roll.total}
      </span>
  ...
  return (
    <div className="mt-6">
      ...
      {campaignRolls.length === 0 ? (
        <p className="text-xs text-zinc-600 py-2">No rolls yet this session.</p>
      ) : (
