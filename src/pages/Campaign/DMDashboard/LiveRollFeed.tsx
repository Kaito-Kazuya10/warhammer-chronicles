...
function RollEntry({ roll }: { roll: CampaignRoll }) {
  ...
  return (
    ...
    >
      <span className="text-[9px] text-slate-600 font-mono shrink-0">{time}</span>
      <div className="flex-1 min-w-0">
        ...
        <span className="text-[10px] text-slate-600">{roll.diceExpression}</span>
      </div>
      <span
        className={`text-sm font-mono font-bold shrink-0 ${
          isNat20 ? 'text-amber-400' : isNat1 ? 'text-red-400' : 'text-slate-200'
        }`}
      >
        {roll.total}
      </span>
  ...
  return (
    <div className="mt-6">
      ...
      {campaignRolls.length === 0 ? (
        <p className="text-xs text-slate-600 py-2">No rolls yet this session.</p>
      ) : (
