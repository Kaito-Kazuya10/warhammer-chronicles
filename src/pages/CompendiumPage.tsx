...
function ItemsTab() {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Count */}
      <p className="text-[10px] text-zinc-600 tracking-wider">{items.length} entries</p>
      {/* Item cards */}
...
function ItemCard({ item }: { item: Item }) {
  ...
  return (
    <div className="border border-zinc-700/25 rounded-md overflow-hidden bg-zinc-800/20">
      <button
        className="w-full flex items-center gap-2 px-3 py-2 hover:bg-zinc-700/20 transition-colors text-left"
        onClick={() => setOpen(o => !o)}
      >
        <span className="text-[10px] text-zinc-600 shrink-0 transition-transform duration-150"
          style={{ transform: open ? 'rotate(90deg)' : undefined }}>▶</span>
        <span className="flex-1 min-w-0">
          <span className="font-medium text-sm text-zinc-100">{item.name}</span>
          {statLine && <span className="ml-2 text-xs text-zinc-500">{statLine}</span>}
        </span>
        <Badge className={`text-[9px] py-0 px-1.5 border shrink-0 ${TIER_BADGE[tier]}`}>
          {tier === 'master-crafted' ? 'MC' : tier.toUpperCase()}
        </Badge>
        {item.cost && <span className="text-[10px] text-zinc-600 shrink-0">{item.cost}</span>}
      </button>

      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-zinc-700/20 space-y-2 bg-zinc-800/10">
          {item.description && (
              ...
              <div className="flex flex-wrap gap-1">
                {item.properties.map(p => {
                  ...
                  return tip ? (
                    <Tooltip key={p}>
                      <TooltipTrigger>
                        <span><Badge variant="outline" className="text-[10px] cursor-help border-zinc-600/40 text-zinc-400">{p}</Badge></span>
                      </TooltipTrigger>
                      <TooltipContent>
                    ...
                    <Badge key={p} variant="outline" className="text-[10px] border-zinc-600/40 text-zinc-400">{p}</Badge>
                  )
                })}
              </div>
            </TooltipProvider>
          ...
          {item.itemAbilities && item.itemAbilities.length > 0 && (
            <div className="space-y-1">
              {item.itemAbilities.map((ab, i) => (
                <div key={i} className="rounded border border-zinc-700/30 px-2 py-1.5 bg-zinc-800/30">
                  <p className="text-[11px] font-semibold text-zinc-200 mb-0.5">{ab.name}</p>
                  <p className="text-[10px] text-zinc-400 leading-relaxed">{ab.description}</p>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="space-y-3">
      <input
        ...
        className="w-full px-3 py-2 rounded-md bg-zinc-800/50 border border-zinc-700/40 text-zinc-100 text-sm placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/40 transition-colors"
      />
      <p className="text-[10px] text-zinc-600 tracking-wider">{classes.length} classes</p>
      <div className="space-y-1">
...
function ClassCard({ cls }: { cls: CharacterClass }) {
  ...
  return (
    <div className="border border-zinc-700/25 rounded-md overflow-hidden bg-zinc-800/20">
      <button
        className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-zinc-700/20 transition-colors text-left"
        onClick={() => setOpen(o => !o)}
      >
        <span className="text-[10px] text-zinc-600 shrink-0 transition-transform duration-150"
          style={{ transform: open ? 'rotate(90deg)' : undefined }}>▶</span>
        <span className="flex-1 min-w-0">
          <span className="font-semibold text-sm text-zinc-100">{cls.name}</span>
        </span>
        <Badge className="text-[9px] py-0 px-1.5 border shrink-0 bg-zinc-700/40 text-zinc-400 border-zinc-600/40">
          d{cls.hitDie}
        </Badge>
      </button>

      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-zinc-700/20 space-y-3 bg-zinc-800/10">
          <p className="text-xs text-zinc-400 leading-relaxed">{cls.description}</p>

          {/* Stats row */}
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1">Primary Abilities</p>
            ...
            <div>
              <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1">Saving Throws</p>
              <div className="flex gap-1 flex-wrap">
                {cls.savingThrows.map(a => (
                  <span key={a} className="px-1.5 py-0.5 rounded bg-zinc-700/40 text-zinc-300 text-[10px] border border-zinc-600/30">
                    {ABILITY_SHORT[a] ?? a.toUpperCase()}
                  </span>
                ))}
              </div>
          ...
          {(cls.armorProficiencies?.length || cls.weaponProficiencies?.length) ? (
            <div className="text-xs text-zinc-500 space-y-0.5">
              {cls.armorProficiencies && cls.armorProficiencies.length > 0 && (
                <p><span className="text-zinc-600">Armor:</span> {cls.armorProficiencies.join(', ')}</p>
              )}
              {cls.weaponProficiencies && cls.weaponProficiencies.length > 0 && (
                <p><span className="text-zinc-600">Weapons:</span> {cls.weaponProficiencies.join(', ')}</p>
              )}
            </div>
          ) : null}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {cls.subclasses.map(s => (
                  <span key={s.id} className="text-[10px] px-2 py-0.5 rounded border border-zinc-700/30 bg-zinc-800/40 text-zinc-300">
                    {s.name}
                  </span>
                ))}
              </div>
          ...
          {lvl1Features.length > 0 && (
            <div>
              <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1.5">Level 1 Features</p>
              <div className="space-y-1.5">
                {lvl1Features.map(f => (
                  <div key={f.name} className="rounded border border-zinc-700/30 px-2 py-1.5 bg-zinc-800/30">
                    <p className="text-[11px] font-semibold text-zinc-200">{f.name}</p>
  ...
  return (
    <div className="space-y-3">
      <input
        ...
        className="w-full px-3 py-2 rounded-md bg-zinc-800/50 border border-zinc-700/40 text-zinc-100 text-sm placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/40 transition-colors"
      />
      <p className="text-[10px] text-zinc-600 tracking-wider">{races.length} species</p>
      <div className="space-y-1">
    ...
    <div className="border border-zinc-700/25 rounded-md overflow-hidden bg-zinc-800/20">
      ...
      >
        <span className="text-[10px] text-zinc-600 shrink-0 transition-transform duration-150"
          style={{ transform: open ? 'rotate(90deg)' : undefined }}>▶</span>
        <span className="flex-1 min-w-0">
          <span className="font-semibold text-sm text-zinc-100">{race.name}</span>
          {race.tier === 'advanced' && (
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-zinc-700/20 space-y-3 bg-zinc-800/10">
          <p className="text-xs text-zinc-400 leading-relaxed">{race.description}</p>

          {/* Stats */}
          <div className="flex flex-wrap gap-3 text-xs text-zinc-500">
            <span><span className="text-zinc-600">Speed:</span> {race.speed} ft</span>
            <span><span className="text-zinc-600">Size:</span> {race.size}</span>
            {race.languages.length > 0 && (
              <span><span className="text-zinc-600">Languages:</span> {race.languages.join(', ')}</span>
            )}
          </div>
          ...
          {race.traits.length > 0 && (
            <div>
              <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1.5">Traits</p>
              <div className="space-y-1.5">
                {race.traits.map(t => (
                  <div key={t.name} className="rounded border border-zinc-700/30 px-2 py-1.5 bg-zinc-800/30">
                    <p className="text-[11px] font-semibold text-zinc-200">{t.name}</p>
                    <p className="text-[10px] text-zinc-500 leading-relaxed mt-0.5">{t.description}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-1.5">
                {race.drawbacks.map(d => (
                  <div key={d.name} className="rounded border border-red-500/20 px-2 py-1.5 bg-red-500/5">
                    ...
                    <p className="text-[10px] text-zinc-500 leading-relaxed mt-0.5">{d.description}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="space-y-3">
      <input
        ...
        className="w-full px-3 py-2 rounded-md bg-zinc-800/50 border border-zinc-700/40 text-zinc-100 text-sm placeholder:text-zinc-600 focus:outline-none focus:border-amber-500/40 transition-colors"
      />
      <p className="text-[10px] text-zinc-600 tracking-wider">{backgrounds.length} backgrounds</p>
      <div className="space-y-1">
    ...
    <div className="border border-zinc-700/25 rounded-md overflow-hidden bg-zinc-800/20">
      ...
      >
        <span className="text-[10px] text-zinc-600 shrink-0 transition-transform duration-150"
          style={{ transform: open ? 'rotate(90deg)' : undefined }}>▶</span>
        <span className="flex-1 min-w-0">
          <span className="font-semibold text-sm text-zinc-100">{bg.name}</span>
          {bg.special && (
            <span className="ml-2 text-[9px] text-red-400/70 border border-red-500/20 rounded px-1 py-0.5">GM APPROVAL</span>
          )}
        </span>
        <div className="flex gap-0.5 shrink-0">
          {bg.skillProficiencies.map(s => (
            <span key={s} className="text-[9px] px-1 py-0.5 rounded bg-zinc-700/40 text-zinc-400 border border-zinc-600/30 capitalize">
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-zinc-700/20 space-y-3 bg-zinc-800/10">
          ...
          {/* Feature */}
          <div className="rounded border border-zinc-700/30 px-2 py-1.5 bg-zinc-800/30">
            <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1">Background Feature</p>
            <p className="text-[11px] font-semibold text-zinc-200">{bg.feature.name}</p>
            <p className="text-[10px] text-zinc-400 leading-relaxed mt-0.5">{bg.feature.description}</p>
          </div>
          ...
          {(bg.toolProficiencies?.length || bg.languages?.length || bg.languageChoices) ? (
            <div className="text-xs text-zinc-500 space-y-0.5">
              {bg.toolProficiencies && bg.toolProficiencies.length > 0 && (
                <p><span className="text-zinc-600">Tools:</span> {bg.toolProficiencies.join(', ')}</p>
              )}
              {bg.languages && bg.languages.length > 0 && (
                <p><span className="text-zinc-600">Languages:</span> {bg.languages.join(', ')}</p>
              )}
          ...
          {bg.startingEquipment.length > 0 && (
            <div>
              <p className="text-[9px] uppercase tracking-widest text-zinc-600 mb-1">Starting Equipment</p>
    ...
    <div className="min-h-screen bg-[#111114]">
      ...
      >
        <div className="flex items-center gap-0 h-12">
          <button
            onClick={() => navigate('/')}
            className="flex items-center gap-1.5 px-4 h-12 text-zinc-400 hover:text-amber-300 hover:bg-white/5 transition-colors shrink-0 border-r border-white/[0.07] text-[11px] tracking-[0.15em] uppercase font-medium"
          >
            ← Home
          </button>
          <h1 className="px-4 text-[11px] tracking-[0.2em] uppercase font-semibold text-zinc-300">
            Compendium
          </h1>
        </div>
